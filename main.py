...
async def get_all_balance_info(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        await tg_helper.reply_text(update, app, "An error occurred while fetching balance information.")
...
async def upsert_balance(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if len(args) != 2:
        await tg_helper.reply_text(update, app, "Please provide two arguments: <limit> <type>.")
    ...
    try:
        ...
        await tg_helper.reply_text(update, app, "An error occurred while setting the balance.")
...
async def change_limit(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    args = update.message.text.strip().split()[1:]
    if len(args) != 2:
        await tg_helper.reply_text(update, app, "Please provide two arguments: <new_limit> <type>.")
    ...
    try:
        ...
        await tg_helper.reply_text(update, app, "An error occurred while changing the limit.")
...
async def delete_balance(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        await tg_helper.reply_text(update, app, "An error occurred while deleting the balance.")
...
async def reset_limits(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        await tg_helper.reply_text(update, app, "An error occurred while resetting the limits.")
...
async def spend(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        if len(parts) != 2:
            await tg_helper.reply_text(update, app, "Please send a message with a number and type, e.g., '50 groceries'.")
        ...
        if new_balance is None:
            ...
            await tg_helper.reply_text(update, app, f"Spent {amount} for '{type}'. Current balance is {new_balance}.")
            logger.info(f"Spent {amount} from type '{type}'. New balance: {new_balance}")
    except ValueError:
        await tg_helper.reply_text(update, app, "Please send a valid number followed by the type, e.g., '50 groceries'.")
